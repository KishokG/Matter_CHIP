...
def build_bundle(cfg: dict, output_dir: Path) -> tuple[Path, str]:
    ...
    readme.write_text(
        ...
        f"    libgirepository1.0-dev, libcairo2-dev, libpcsclite-dev, pcscd, swig\n"
        f"\n"
        f"Troubleshooting\n"
        f"───────────────\n"
    ...
    install_sh.write_text(
        ...
        f"  libcairo2-dev \\\n"
        f"  libpcsclite-dev pcscd swig --quiet\n"
        f"# libpcsclite-dev + swig: build deps for pyscard (in requirements.nfc.txt) —\n"
        f"# without them the wheel fails with 'winscard.h: No such file' /\n"
        f"# 'libpcsclite not found' on a fresh host. pcscd: PC/SC runtime for NFC.\n"
