...
activate_env() {
    ...
    # .environment/activate.sh is generated by bootstrap.sh and has all
    # PW_* variables pre-exported — safe to source in non-interactive shells.
    # This is different from scripts/activate.sh which expects PW_* to already
    # be set in the shell session.
    local ENV_ACTIVATE="${SDK_DIR}/.environment/activate.sh"

    if [[ ! -f "${ENV_ACTIVATE}" ]]; then
        fail ".environment/activate.sh not found at ${ENV_ACTIVATE}
  → The SDK has not been bootstrapped yet.
  → Re-run without --skip-sdk so bootstrap runs first."
    fi
