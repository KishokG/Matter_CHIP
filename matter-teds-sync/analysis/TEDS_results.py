...
# come from the "analyses" list in config/releases.json so this script can
# run against any release without being edited.
SERVICE_ACCOUNT_FILE = os.environ.get("SERVICE_ACCOUNT_FILE", "credentials.json")
CONFIG_PATH = os.environ.get(
    "RELEASES_CONFIG_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "config", "releases.json"),
)

# Section headers — internal labels used to group rows in the Summary sheet,
# not user-facing config, so these stay as script constants.
...
def load_analyses_config():
    ...
    if not analyses:
        raise ValueError(f"No 'analyses' entries found in {CONFIG_PATH}")
    required_fields = [
        "name", "sheetId", "sourceSheetName", "masterTcSheet",
        "summarySheetName", "deltaSheetName",
    ]
...
def run_analysis(cfg, client):
    """Run the full SVE results analysis for one config entry."""
    name = cfg["name"]
    ...
    col_company_id = columns.get("companyId", 3)
    col_dut_id = columns.get("dutId", 4)
    ...
    # ── Read test data from the master TC sheet ──────────────────────────────
    ...
    if not old_summary_data:
        # First run — no previous summary existed to compare against
